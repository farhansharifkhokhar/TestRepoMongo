...
import com.stepsolar.sr.util.DateFormatProvider;
...
public class TokenHelper {
	...
	public String generateToken(String username, String userId, Device device) {
		...
		return Jwts.builder().setIssuer(APP_NAME).setSubject(username).setId(userId).setAudience(audience)
				.setIssuedAt(DateFormatProvider.now()).setExpiration(generateExpirationDate(device))
				.signWith(SIGNATURE_ALGORITHM, SECRET).compact();
	}

	public String generateTokenForUserActivation(String username, String userId) {

		return Jwts.builder().setIssuer(APP_NAME).setSubject(username).setId(userId)
				.setIssuedAt(DateFormatProvider.now()).signWith(SIGNATURE_ALGORITHM, SECRET).compact();
	}
	...
	private Date generateExpirationDate(Device device) {
		...
		long expiresIn = device.isTablet() || device.isMobile() ? convertedTimeMobileTokenExpireTime
				: convertedTimeTokenExpireTime;
	...
	public int getExpiredIn(Device device) {
		int convertedTimeTokenExpireTime = Integer.parseInt(StringUtils.deleteWhitespace(EXPIRES_IN));
		int convertedTimeMobileTokenExpireTime = Integer.parseInt(StringUtils.deleteWhitespace(MOBILE_EXPIRES_IN));
		return device.isMobile() || device.isTablet() ? convertedTimeMobileTokenExpireTime
				: convertedTimeTokenExpireTime;
	}
